...
Timeline.init = function(containerRef, wavesurfer) {
	...
	// Create canvas
	this.container = containerRef; //$("#timeline-container");
...
Timeline.mouseMoved = function(e) {
	...
	if(t.state.draggingTime) {
        ...
        // @NEEDTOFIX
		Timeline.wavesurfer.seekTo(t.timePercent);
        ...
        // @NEEDTOFIX
		Timeline.wavesurfer.setScroll(Math.ceil(t.timeViewPosition * t.timeScale));
	}
...
Timeline.mouseWheel = function(e) {
    ...
    // @NEEDTOFIX
	Timeline.wavesurfer.zoom(t.timeScale);
	Timeline.wavesurfer.setScroll(t.timeViewPosition * t.timeScale);
...
export function timelineRenderUpdate() {
...
Timeline.updateTime = function() {
    ...
    // @NEEDTOFIX
	if(Timeline.wavesurfer.isPlaying()) {
		this.time = Timeline.wavesurfer.getCurrentTime();
		//this.updateActiveTracks();
	}
...
Timeline.startScalingKeyframes = function() {
	...
	var first = 999;
	var last = 0;
	for(let k in Timeline.selectedKeyframes) {
